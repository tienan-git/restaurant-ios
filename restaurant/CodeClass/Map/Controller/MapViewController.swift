...
class MapViewController: UIViewController {
    ...
    func isLocationEnable() -> Bool {
        return CLLocationManager.locationServicesEnabled() && CLLocationManager.authorizationStatus() == CLAuthorizationStatus.authorizedWhenInUse
    }
    ...
    func routeGuideOrOnlyForClose() {
        ...
        if isLocationEnable(){
            ...
            UtilClass.alertViewShowWithoutCancel(vc: self,
                                                 title: "位置情報の設定をオンにしてください",
                                                 message: "[手順] 1.iPhoneの「設定」を開く > 2.「プライバシー」を選択 > 3.「位置情報サービス」を選択 > 4.オンにする",
                                                 sureHandler: nil)
        }
