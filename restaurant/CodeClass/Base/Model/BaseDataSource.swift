...
open class BaseDataSource: NSObject {
    ...
    required override public init() {
        ...
        if self.cachedData() != nil {
            let iscount = self.count()
            dataSource?.insert(self.cachedData()! as AnyObject, at: iscount!)
        } else {
            dataSource? = Array()
        }
    ...
    @objc func shouldRefreshDataSource(_ notification: Foundation.Notification){
        self.delegate?.completeRequest()
        self.delegate?.completeRequest?(self)
    }
    
    @objc func removeMyExistenseNotification(_ notification: Foundation.Notification){
