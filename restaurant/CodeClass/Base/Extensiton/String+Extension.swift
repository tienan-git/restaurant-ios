...
//        let attributeDic: Dictionary<NSAttributedString.Key, UIFont> = [NSAttributedString.Key(rawValue: NSAttributedString.Key.font.rawValue): font]
...
//    }
    ...
    fileprivate func convertFullWidthToHalfWidth(_ reverse: Bool) -> String {
        var string: String = ""
        let chars = self.map{ String(describing: $0) }
        chars.forEach {
    ...
    func removed(character: Character) -> String {
        return self.filter { $0 != character }.map { String($0) }.joined(separator: "")
    }
...
//        }
        ...
        guard let endIndex = self.index(startIndex, offsetBy: string.count, limitedBy: self.endIndex) else {
            return nil
        }
